...
class ALSModel:
    def __init__(self, f, reg_param, iters, a, article_url_list):
        # 모델 및 학습 준비
        print("Initialize..")
        ...
        # BigQuery 클라이언트 설정
        bigquery_key_path = os.environ.get('S3_BIGQUERY_KEY_PATH')
        if not bigquery_key_path:
            raise ValueError("S3_BIGQUERY_KEY_PATH environment variable is not set")
    ...
    def make_user_item_matrix(self, user_log_df: pd.DataFrame):
        ...
        data=np.zeros((len(unique_user_names), len(self.article_urls))),  # 유저 수 x 게시글 크기의 0으로 초기화된 배열
        index=unique_user_names,  # 행 인덱스를 유저 이름으로 설정
        columns=self.article_urls      # 열 이름을 URL로 설정
        )
    ...
    def transform_item_latent_matrix(self):
        ...
        article_urls = self.article_info_df['url'].tolist()  # URL 리스트로 변환
        self.item_latent_matrix.insert(0, 'url', self.article_urls)
    ...
    def save_data(self):
        ...
        # 환경 변수에서 저장 경로 가져오기
        save_path = os.environ.get('TRAINED_MODEL_PATH')
        if not save_path:
            raise ValueError("TRAINED_MODEL_PATH environment variable is not set")

        file_path = os.path.join(save_path, 'trained_article_latent_matrix.pkl')
